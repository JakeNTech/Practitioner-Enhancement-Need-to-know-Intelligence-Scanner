# Description: Microsoft Office 2007+ Word Documents.
# Author: @JakeNTech
...
def docx(file):
    ...
    try:
        ...
        try:
            this_line.append(str(doc.getElementsByTagName('dc:title')[0].childNodes[0].data).replace(',','.'))
        except:
            this_line.append("N/A")

        try:
            this_line.append(str(doc.getElementsByTagName('dc:description')[0].childNodes[0].data).replace(',','.'))
        except:
            this_line.append("N/A")
        ...
        this_line = ["Failed_to_Load","N/A","N/A","N/A","N/A","N/A"]
...
__artifacts__ = {
    "vnd.openxmlformats-officedocument.wordprocessingml.document": (
        "File Analysis",
        "title,subject,author,last_modified_by,date_created,date_modified",
        docx)
}